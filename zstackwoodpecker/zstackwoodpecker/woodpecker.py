                ...
                if xmlobject.has_element(s, self.CASE_TAG):
                    for c in s.get_child_node_as_list(self.CASE_TAG):
                        ...
                        if not case.name:
                            ...
                            if suite.path != None:
                                case.name = case.path[len(suite.path)+1:][:-3] + "::" + case.flavor
                            elif suite.setup_case != None:
                                case.name = case.path[len(os.path.dirname(suite.setup_case.path))+1:][:-3] + "::" + case.flavor
                            else:
                                case.name = '/'.join(case.path.split('/')[-2:])[:-3] + "::" + case.flavor
                        case.suite = suite
