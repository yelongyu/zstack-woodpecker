        ...
        def run_case(suite, case, suite_repeat, case_repeat, parallel=0):
            ...
            try:
                if case_name == 'suite_setup' or self.startDebugger == False:
                    test_env_variables = 'WOODPECKER_SCENARIO_CONFIG_FILE=%s WOODPECKER_SCENARIO_FILE=%s WOODPECKER_SCENARIO_DESTROY=%s WOODPECKER_TEST_CONFIG_FILE=%s WOODPECKER_CASE_ACTION_LOG_PATH=%s WOODPECKER_NO_ERROR_CLEANUP=%s WOODPECKER_ONLY_ACTION_LOG=%s CASE_FLAVOR=%s' % (self.scenario_config, self.scenario_file, self.scenario_destroy, self._check_test_config(suite.test_config, case.path), case_action_log_path, self.noCleanup, self.onlyActionLog, case.flavor)
                else:
                    test_env_variables = 'WOODPECKER_SCENARIO_CONFIG_FILE=%s WOODPECKER_SCENARIO_FILE=%s WOODPECKER_SCENARIO_DESTROY=%s WOODPECKER_TEST_CONFIG_FILE=%s WOODPECKER_CASE_ACTION_LOG_PATH=%s WOODPECKER_NO_ERROR_CLEANUP=%s WOODPECKER_ONLY_ACTION_LOG=%s WOODPECKER_START_DEBUGGER=%s CASE_FLAVOR=%s' % (self.scenario_config, self.scenario_file, self.scenario_destroy, self._check_test_config(suite.test_config, case.path), case_action_log_path, self.noCleanup, self.onlyActionLog, self.startDebugger, case.flavor)
