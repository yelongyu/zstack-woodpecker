...
class zstack_kvm_snapshot_checker(checker_header.TestChecker):
    def check(self):
        ...
        if 'auto' in self.test_obj.get_current_snapshot().get_snapshot().name:
            test_util.test_logger('Checking Result: skip snapshot checking, since current snapshot: %s is auto generated by resize' % self.test_obj.get_current_snapshot().get_snapshot().uuid)
            return self.judge(self.exp_result)

        if target_volume.get_volume().uuid in self.test_obj.get_current_snapshot().get_snapshot().name:
            test_util.test_logger('Checking Result: skip snapshot checking, since current snapshot: %s is auto generated by clone or create_image_from_volume' % self.test_obj.get_current_snapshot().get_snapshot().uuid)
            return self.judge(self.exp_result)

        if target_volume.get_target_vm():
            if target_volume.get_target_vm().get_vm().uuid in self.test_obj.get_current_snapshot().get_snapshot().name:
                test_util.test_logger('Checking Result: skip snapshot checking, since current snapshot: %s is auto generated by reinit' % self.test_obj.get_current_snapshot().get_snapshot().uuid)
                return self.judge(self.exp_result)
