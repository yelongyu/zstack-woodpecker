    ...
    def get_checking_points(self, snapshot):
        ...
        for checking_snapshot in parents_snapshots:
            ...
            if checking_snapshot.get_target_volume().get_volume().uuid in checking_snapshot.get_snapshot().name:
                test_util.test_logger('Update Checking points: skip snapshot update, since %s is auto generated by clone or create_image_from_volume' % checking_snapshot.get_snapshot().uuid)
                continue
            if checking_snapshot.get_target_volume().get_target_vm():
