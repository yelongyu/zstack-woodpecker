...
class zstack_vcenter_pf_rule_exist_checker(checker_header.TestChecker):
    '''
...
class zstack_vcenter_pf_tcp_checker(checker_header.TestChecker):
    '''check vcenter vm dnat status. If VM is reachable from external target, 
...
class zstack_vcenter_pf_udp_checker(checker_header.TestChecker):
    '''check vcenter vm dnat status. If VM is reachable from external target, 
        return self.judge(True). If not, return self.judge(False)'''
    ...
    def check(self):
        ...
        super(zstack_vcenter_pf_udp_checker, self).check()
        return self.judge(self.exp_result)

class zstack_vcenter_pf_vip_icmp_checker(checker_header.TestChecker):
