'''
Zstack VCenter Checker Factory.        
...
import zstackwoodpecker.zstack_test.vcenter_checker.zstack_vcenter_volume_checker as volume_checker
import zstackwoodpecker.zstack_test.vcenter_checker.zstack_vcenter_image_checker as image_checker
import zstackwoodpecker.zstack_test.vcenter_checker.zstack_vcenter_security_group_checker as sg_checker
import zstackwoodpecker.zstack_test.vcenter_checker.zstack_vcenter_port_forwarding_checker as pf_checker
import zstackwoodpecker.zstack_test.vcenter_checker.zstack_vcenter_host_checker as host_checker
import zstackwoodpecker.zstack_test.vcenter_checker.zstack_vcenter_eip_checker as eip_checker
import zstackwoodpecker.zstack_test.vcenter_checker.zstack_vcenter_vip_checker as vip_checker
import zstackwoodpecker.zstack_test.vcenter_checker.zstack_vcenter_snapshot_checker as sp_checker
import zstackwoodpecker.zstack_test.vcenter_checker.zstack_vcenter_load_balancer_checker as lb_checker
import zstackwoodpecker.test_util as test_util
import apibinding.inventory as inventory

class VCenterVmCheckerFactory(checker_header.CheckerFactory):
    def create_checker(self, test_obj): 
        vcenter_vm_checker_chain = checker_header.CheckerChain()
        checker_dict = {}

        if test_obj.state == vm_header.RUNNING:
            ##checker_dict[vm_checker.zstack_vcenter_vm_set_host_vlan_ip] = True
            checker_dict[db_checker.zstack_vm_db_checker] = True
            checker_dict[vm_checker.zstack_vcenter_vm_running_checker] = True
            ##TODO:
            ##if behind of VR
            ...
            ##        checker_dict[vm_checker.zstack_vcenter_vm_dhcp_checker] = True
            ##        checker_dict[vm_checker.zstack_vcenter_vm_network_checker] = True
            ##        #if guest can't get IP address from DHCP, auto case can
            ##        # not test DNS feature.
            ##        if 'DNS' in svr_types:
            ##            checker_dict[vm_checker.zstack_vcenter_vm_dns_checker] \
            ##                    = True
            ##        else:
            ##            checker_dict[vm_checker.zstack_vcenter_vm_dns_checker] \
            ##                    = False
            ##    else:
            ##        checker_dict[vm_checker.zstack_vcenter_vm_dhcp_checker] = False
            ##        checker_dict[vm_checker.zstack_vcenter_vm_network_checker] \
            ##                = False
            ##    if 'SNAT' in svr_types:
            ##        checker_dict[vm_checker.zstack_vcenter_vm_snat_checker] = True
            ##    else:
            ##        checker_dict[vm_checker.zstack_vcenter_vm_snat_checker] = False
            ##    #if 'PortForwarding' in svr_types:
            ##    #    checker_dict[vm_checker.zstack_vcenter_vm_dnat_checker] = True
            ##    #else:
            ##    #    checker_dict[vm_checker.zstack_vcenter_vm_dnat_checker] = False
            ##else:
            ##    sp_types = test_lib.lib_get_vm_l3_service_provider_types(test_obj.vm)
            ##    if 'Flat' in sp_types:
            ##        checker_dict[vm_checker.zstack_vcenter_vm_ssh_no_vr_checker] = True

            ##if test_obj.get_creation_option().get_default_l3_uuid():
            ##    checker_dict[vm_checker.zstack_vcenter_vm_default_l3_checker] = True

        elif test_obj.state == vm_header.STOPPED:
            checker_dict[db_checker.zstack_vm_db_checker] = True
            #stopped_checker is deprecated, since the stopped vm will be removed
            #from host.
            #checker_dict[vm_checker.zstack_vcenter_vm_stopped_checker] = True

        elif test_obj.state == vm_header.PAUSED:
            checker_dict[db_checker.zstack_vm_db_checker] = True
            checker_dict[vm_checker.zstack_vcenter_vm_suspended_checker] = True
            ...
            checker_dict[db_checker.zstack_vm_db_checker] = True
            checker_dict[vm_checker.zstack_vcenter_vm_destroyed_checker] = True
        elif test_obj.state == vm_header.EXPUNGED:
            checker_dict[db_checker.zstack_vm_db_checker] = True

        vcenter_vm_checker_chain.add_checker_dict(checker_dict, test_obj)
        return vcenter_vm_checker_chain

class VCenterVolumeCheckerFactory(checker_header.CheckerFactory):
    def create_checker(self, test_obj): 
        vcenter_volume_checker_chain = checker_header.CheckerChain()
        checker_dict = {}
        if test_obj.state == volume_header.CREATED:
            checker_dict[db_checker.zstack_volume_db_checker] = True
            checker_dict[volume_checker.zstack_vcenter_volume_file_checker] = False
        ...
        vcenter_volume_checker_chain.add_checker_dict(checker_dict, test_obj)
        return vcenter_volume_checker_chain

class VCenterImageCheckerFactory(checker_header.CheckerFactory):
    def create_checker(self, test_obj): 
        vcenter_image_checker_chain = checker_header.CheckerChain()
        checker_dict = {}
        if test_obj.state == image_header.CREATED:
            checker_dict[db_checker.zstack_image_db_checker] = True
            checker_dict[image_checker.zstack_vcenter_image_file_checker] = True
        ...
        if test_obj.state == image_header.EXPUNGED:
            ...
            checker_dict[image_checker.zstack_vcenter_image_file_checker] = False

        vcenter_image_checker_chain.add_checker_dict(checker_dict, test_obj)
        return vcenter_image_checker_chain

class VCenterSecurityGroupCheckerFactory(checker_header.CheckerFactory):
    def create_checker(self, test_obj): 
        vcenter_sg_checker_chain = checker_header.CheckerChain()
        checker_dict = {}
        for nic_uuid in test_obj.get_all_nics():
            ...
            if target_vm.state == vm_header.RUNNING:
                if test_lib.lib_is_vm_sim(target_vm.vm):
                    vcenter_sg_checker_chain.add_checker(db_checker.zstack_sg_db_checker(True), test_obj)
                    continue
                if not test_lib.lib_is_vm_vcenter(target_vm.vm):
                    continue
                ...
                if test_obj.get_nic_tcp_ingress_rules(nic_uuid):
                    ...
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)

                    checker = sg_checker.zstack_vcenter_sg_tcp_ingress_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)

                    checker = sg_checker.zstack_vcenter_sg_tcp_internal_vms_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)
                else:
                    checker = sg_checker.zstack_vcenter_sg_tcp_ingress_exist_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, False, test_obj)

                if test_obj.get_nic_tcp_egress_rules(nic_uuid):
                    checker = sg_checker.zstack_vcenter_sg_tcp_egress_exist_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)

                    checker = sg_checker.zstack_vcenter_sg_tcp_egress_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)
                ...
                if test_obj.get_nic_udp_ingress_rules(nic_uuid):
                    checker = sg_checker.zstack_vcenter_sg_udp_ingress_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)
                else:
                    checker = sg_checker.zstack_vcenter_sg_udp_ingress_checker()
                ...
                if test_obj.get_nic_udp_egress_rules(nic_uuid):
                    checker = sg_checker.zstack_vcenter_sg_udp_egress_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)
                else:
                    checker = sg_checker.zstack_vcenter_sg_udp_egress_checker()
                ...
                if test_obj.get_nic_icmp_ingress_rules(nic_uuid):
                    checker = sg_checker.zstack_vcenter_sg_icmp_ingress_exist_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)

                    checker = sg_checker.zstack_vcenter_sg_icmp_ingress_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)

                    checker = sg_checker.zstack_vcenter_sg_icmp_internal_vms_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)
                else:
                    checker = sg_checker.zstack_vcenter_sg_icmp_ingress_exist_checker()
                ...
                if test_obj.get_nic_icmp_egress_rules(nic_uuid):
                    checker = sg_checker.zstack_vcenter_sg_icmp_egress_exist_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)

                    checker = sg_checker.zstack_vcenter_sg_icmp_egress_checker()
                    checker.set_nic_uuid(nic_uuid)
                    vcenter_sg_checker_chain.add_checker(checker, True, test_obj)
        ...
        for test_vm in test_obj.get_detached_vm():
            vm = test_vm.vm
            if not test_lib.lib_is_vm_vcenter(vm):
                continue
            checker = sg_checker.zstack_vcenter_sg_tcp_ingress_exist_checker()
            checker.set_vm(vm)
            vcenter_sg_checker_chain.add_checker(checker, False, test_obj)
        ...
        return vcenter_sg_checker_chain

class VCenterPortForwardingCheckerFactory(checker_header.CheckerFactory):
    def create_checker(self, test_obj): 
        vcenter_pf_checker_chain = checker_header.CheckerChain()
        checker_dict = {}
        ...
        if test_obj.get_state() == pf_header.ATTACHED and \
            ...
            if pf_rule.protocolType == inventory.TCP:
                checker_dict[pf_checker.zstack_vcenter_pf_tcp_checker] = True
            if pf_rule.protocolType == inventory.UDP:
                checker_dict[pf_checker.zstack_vcenter_pf_rule_exist_checker] = True

        elif test_obj.get_state() == pf_header.ATTACHED and test_obj.get_target_vm().get_state() == vm_header.STOPPED:
            checker_dict[pf_checker.zstack_vcenter_pf_vip_icmp_checker] = False
            if pf_rule.protocolType == inventory.TCP:
                checker_dict[pf_checker.zstack_vcenter_pf_tcp_checker] = False

        elif test_obj.get_state() == pf_header.DETACHED:
            checker_dict[pf_checker.zstack_vcenter_pf_vip_icmp_checker] = False

        vcenter_pf_checker_chain.add_checker_dict(checker_dict, test_obj)
        return vcenter_pf_checker_chain

class HostCheckerFactory(checker_header.CheckerFactory):
    def create_checker(self, test_obj): 
        host_checker_chain = checker_header.CheckerChain()
        checker = host_checker.zstack_vcenter_host_checker()
        ...
        if test_obj.get_state() != lb_header.DELETED:
            ...
            for lbl in test_obj.get_load_balancer_listeners().values():
                if lbl.get_state() != lb_header.DELETED:
                    checker = lb_checker.zstack_vcenter_lbl_checker()
                    checker.set_lbl(lbl)
