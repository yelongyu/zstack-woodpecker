...
ansible-playbook virtualrouter.yaml -i $ansible_inventory -e "gather_facts=No zstack_root=/var/lib/zstack/ pkg_virtualrouter=virtualrouter-0.8.tar.gz pypi_url=$ZSTACK_PYPI_URL trusted_host=$PYPI_TRUSTED_HOST pkg_zstacklib=zstacklib-0.8.tar.gz host=chroot chroot=true" || exception "Update Virtual Router Failed"

echo_bold "Update Virtual Router Successfully"

echo_bold "Update Appliance VM service..."
ansible-playbook appliancevm.yaml -i $ansible_inventory -e "zstack_root=/var/lib/zstack/ pkg_appliancevm=appliancevm-0.8.tar.gz pypi_url=$ZSTACK_PYPI_URL trusted_host=$PYPI_TRUSTED_HOST pkg_zstacklib=zstacklib-0.8.tar.gz host=chroot chroot=true" || exception "Update Appliance VM Failed"
...
if [ ! -z $ZSTACK_TEST_AGENT_PATH ]; then
    ...
    ansible-playbook testagent.yaml -i $ansible_inventory -e "zstack_root=/var/lib/zstack/ pkg_testagent=zstacktestagent-0.1.0.tar.gz pypi_source_tar=pypi.tar.bz pkg_zstacklib=zstacklib-0.8.tar.gz host=chroot chroot=true" || ( rm -rf $tmpfile && exception "Update Test Agent Failed")
    rm -rf $tmpfile
