...
Test about monitor trigger on host network tx in five minutes
...
def test():
    ...
    host = hosts[0]
    duration = 300
    expression = "host.network.io{direction=\"tx\"} > 200"
    ...
    t = threading.Thread(target=test_stub.run_network_rx,args=(ssh_cmd,vm_ip,))
    t.start()
    #time.sleep(360)
    time.sleep(320)
