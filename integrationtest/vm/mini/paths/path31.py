import zstackwoodpecker.test_state as ts_header
TestAction = ts_header.TestAction
def path():
    return dict(initial_formation="template5", path_list=[
        [TestAction.create_vm_by_image, "iso1", "iso", "vm1"],
        [TestAction.reboot_vm, "vm1"],
        [TestAction.create_volume, "volume1", "=scsi,thin"],
        [TestAction.attach_volume, "vm1", "volume1"],
        [TestAction.create_volume_backup, "volume1", "backup1"],
        [TestAction.stop_vm, "vm1"],
        [TestAction.create_image_from_volume, "vm1", "vm1-image1"],
        [TestAction.create_volume, "volume2", "=scsi,thin"],
        [TestAction.attach_volume, "vm1", "volume2"],
        [TestAction.create_mini_vm, "vm2", 'data_volume=false', 'cpu=random', 'memory=2', 'network=random',
         'provisiong=thick'],
        [TestAction.delete_volume, "volume1"],
        [TestAction.recover_volume, "volume1"],
        [TestAction.delete_image, "vm1-image1"],
        [TestAction.expunge_image, "vm1-image1"],
        [TestAction.start_vm, "vm1"],
        [TestAction.create_volume_backup, "volume2", "backup2"],
        [TestAction.create_mini_vm, "vm3", 'data_volume=true', 'cpu=2', 'memory=2', 'provisiong=thick'],
        [TestAction.resize_volume, "vm1", 5 * 1024 * 1204],
        [TestAction.resize_volume, "vm2", 5 * 1024 * 1204],
        [TestAction.resize_volume, "vm3", 5 * 1024 * 1204],
        [TestAction.create_volume, "volume3", "size=random", "flag=scsi,thin"],
        [TestAction.create_volume, "volume4", "size=random", "flag=scsi,thin"],
        [TestAction.create_mini_vm, "vm4", 'data_volume=false', 'cpu=random', 'memory=2', 'provisiong=thick'],
        [TestAction.add_image, "image1", 'root', "http://172.20.1.28/mirror/diskimages/centos_vdbench.qcow2"],
        [TestAction.delete_volume, "volume1"],
        [TestAction.expunge_volume, "volume1"],
        [TestAction.create_vm_by_image, "iso1", "iso", "vm5"],
        [TestAction.create_vm_backup, "vm5", "backup3"],
        [TestAction.resize_volume, "vm1", 5 * 1024 * 1204],
        [TestAction.resize_volume, "vm2", 5 * 1024 * 1204],
        [TestAction.resize_volume, "vm3", 5 * 1024 * 1204],
        [TestAction.resize_volume, "vm5", 5 * 1024 * 1204],
        [TestAction.resize_volume, "vm4", 5 * 1024 * 1204],
        [TestAction.delete_image, "image1"],
        [TestAction.delete_vm_backup, "backup3"],
        [TestAction.add_image, "image2", 'root', "http://172.20.1.28/mirror/diskimages/centos_vdbench.qcow2"],
        [TestAction.attach_volume, "vm5", "volume3"]])