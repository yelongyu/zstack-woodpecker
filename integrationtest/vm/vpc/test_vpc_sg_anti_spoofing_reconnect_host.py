...
Test host reconnection when anti-spoofing is globally enabled.
...
def test():
    ...
    vm_ops.set_vm_clean_traffic(vm1.vm.uuid)
    vm_ops.set_vm_clean_traffic(vm2.vm.uuid)
    ...
    #reconnect host
    for i in range(0,3):
        host = res_ops.query_resource(res_ops.HOST)[i]
        host_uuid = host.uuid
        for j in range(0,4):
            ...
            if host_status != "Connected":
                test_util.test_fail('Fail to Reconnect Host %s after 60s.' % host_uuid)
