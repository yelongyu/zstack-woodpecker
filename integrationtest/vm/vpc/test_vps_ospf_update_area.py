...
ospf_area_id = '1.1.1.100'
ospf_area_type = 'Standard'
area_auth_type = 'Plaintext'
...
def test():
    ...
    test_util.test_dsc("update ospf area")
    vpc_ops.update_vrouter_ospf_area(uuid=area_uuid, areaType='Standard', areaAuth='MD5', password='123456', keyId=50)
    test_util.test_dsc("check ospf neighbor state")
    for vr in vr_uuid:
        if vpc_ops.get_vrouter_ospf_neighbor(vr):
            if 'Full' not in vpc_ops.get_vrouter_ospf_neighbor(vr)[0]['state']:
    ...
    vpc_ops.update_vrouter_ospf_area(uuid=area_uuid, areaType='Stub', areaAuth='MD5', password='223456', keyId=10)
    test_util.test_dsc("check ospf neighbor state")
    for vr in vr_uuid:
        if vpc_ops.get_vrouter_ospf_neighbor(vr):
            if 'Full' not in vpc_ops.get_vrouter_ospf_neighbor(vr)[0]['state']:
    ...
    vpc_ops.update_vrouter_ospf_area(uuid=area_uuid, areaType='NSSA', areaAuth='Plaintext', password='323456')
    test_util.test_dsc("check ospf neighbor state")
    for vr in vr_uuid:
        if vpc_ops.get_vrouter_ospf_neighbor(vr):
            if 'Full' not in vpc_ops.get_vrouter_ospf_neighbor(vr)[0]['state']:
