...
Test detach an EIP from a VM and then attach it to another VM.

Test step:
    1. Create 2 VM
    2. Create a EIP with one VM.
    3. Detach EIP from the VM.
    4. Attach EIP to another VM.
    5. Check the connectivity of the EIP.
...
def test():
    ...
    '''
    test_util.test_dsc('Access the other VM via EIP.')
    ...
    eip = test_stub.create_eip('create eip test', vip_uuid=vip.get_vip().uuid, vnic_uuid=vm_nic_uuid, vm_obj=vm1)
    vip.attach_eip(eip)
    vip.check()
    ...
    test_util.test_pass('Switch EIP Success')
