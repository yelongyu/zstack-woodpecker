...
#    default_mode = conf_ops.get_global_config_value('kvm', 'videoType')
    ...
    if vm_mode != 'cirrus':
        test_util.test_fail('VM is expected to work in cirrus mode instead of %s' % (vm_mode))
    ...
    box = image.getbbox()
    if box != (0, 18, 359, 80):
        test_util.test_fail("VM is expected to display text in area (0, 18, 359, 80) while it's actually: (%s, %s, %s, %s)" % (box[0], box[1], box[2], box[3]))
