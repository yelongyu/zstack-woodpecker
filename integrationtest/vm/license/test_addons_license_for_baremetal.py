...
def test():
    ...
    test_util.test_logger('Load and Check Hybrid license with 10 day and 2 CPU')
    file_path = test_stub.gen_license('woodpecker', 'woodpecker@zstack.io', '10', 'Hybrid', '2', '')
    test_stub.load_license(file_path)
    issued_date = test_stub.get_license_info().issuedDate
    expired_date = test_stub.license_date_cal(issued_date, 86400 * 10)
    test_stub.check_license("woodpecker@zstack.io", 2, None, False, 'Hybrid', issued_date=issued_date, expired_date=expired_date)
