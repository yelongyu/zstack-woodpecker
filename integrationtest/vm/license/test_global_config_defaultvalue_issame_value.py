...
New Integration Test for global config defaultValue is same value.
@Antony yetian 
@date: 2019-05-20 
...
def sendMsg_to_Ding():
	...
	data = {
			...
			"msgtype": "text",
			"text": {
					"content": "1.zstack global config defaultValue is different value on latest nightly test."
				}
		}
...
def check(file1_path,file2_path):
	...
	if output_result[0] != 0:
		if output_result[1].strip():
			sendMsg_to_Ding()
			test_util.test_fail("global config defaultValue is different value ,more detalis as follow\n %s" %(output_result[1]))

def verify_config(management_ip):
	tmp_file = '/tmp/%s' % uuid.uuid1().get_hex()
	test_util.test_logger("%s" %(tmp_file))
	tmp_file1 = '/tmp/%s' % uuid.uuid1().get_hex()
	test_util.test_logger("%s" %(tmp_file1))
	test_lib.lib_execute_ssh_cmd(management_ip, 'root', 'password', 'zstack-cli LogInByAccount accountName=admin password=password', 180)
	test_lib.lib_execute_ssh_cmd(management_ip,'root','password','zstack-cli QueryGlobalConfig fields=defaultValue sortBy=name |awk \'{print $2}\' |tr -s \'\n\' > %s' %(tmp_file), 180)
	test_lib.lib_execute_ssh_cmd(management_ip,'root','password','zstack-cli QueryGlobalConfig fields=value sortBy=name |awk \'{print $2}\' |tr -s \'\n\' > %s' %(tmp_file1), 180)
	#default_path=environment(management_ip)
	#test_util.test_logger("default global config path:%s" %(default_path))

	test_util.test_logger("check two file diff:%s, %s" %(tmp_file, tmp_file1))
	check(tmp_file,tmp_file1)
...
def test():
	...
	test_util.test_pass('Check Global Config defaultValue is same value Pass')
