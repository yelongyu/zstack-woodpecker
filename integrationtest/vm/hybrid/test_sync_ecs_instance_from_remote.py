...
Test sync ECS instance from Aliyun.
...
#     region_id = datacenter_list[0].regionId
    ...
    time.sleep(5)
    ecs_synced = hyb_ops.sync_ecs_instance_from_remote(datacenter_inv.uuid)
    assert ecs_synced.addList
    ecs_local = hyb_ops.query_ecs_instance_local()
    ecs_uuid_list = [l.uuid for l in ecs_local]
    assert len(ecs_uuid_list) > 1
