...
def test():
    ...
    ecs_id_prepaid = os.getenv('prepaid_ecsInstanceId')
    ...
    time.sleep(5)
    hyb_ops.sync_ecs_instance_from_remote(datacenter_inv.uuid, only_zstack='false')
    time.sleep(5)
    ecs_instance_local = hyb_ops.query_ecs_instance_local()
    for el in ecs_instance_local:
        if el.ecsInstanceId == ecs_id_prepaid:
            ecs_inv = el
    assert ecs_inv.chargeType == 'PrePaid'
