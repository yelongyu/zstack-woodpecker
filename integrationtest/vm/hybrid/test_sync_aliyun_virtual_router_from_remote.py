...
def test():
    ...
    for vpc in vpc_local:
        if vpc.name == 'test-for-ipsec':
            hyb_ops.sync_aliyun_virtual_router_from_remote(vpc.uuid)
    vrouter_local = hyb_ops.query_aliyun_virtual_router_local()
    assert len(vrouter_local) > 0
