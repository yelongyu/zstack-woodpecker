...
#     test_obj_dict.add_vm(vm_inv)
    ...
    assert hyb_ops.sync_vpc_vpn_connection_from_remote(datacenter_inv.uuid)[0].status == 'ike_sa_not_established'
    ...
    time.sleep(30)
    assert hyb_ops.sync_vpc_vpn_connection_from_remote(datacenter_inv.uuid)[0].status == 'ipsec_sa_established'
    hyb_ops.del_vpc_vpn_connection_remote(ipsec_vpn_inv_connection.uuid)
    time.sleep(10)
    ipsec_vpn_inv_connection = create_vpn_connection(vpn_gateway, vswitch_cidr, zstack_cidrs, auth_alg_1='md5', auth_alg_2='md5')
    assert hyb_ops.sync_vpc_vpn_connection_from_remote(datacenter_inv.uuid)[0].status == 'ipsec_sa_not_established'
