...
def test():
    ...
    assert len(vpc_vpn_conn_local) > 0
    for conn in vpc_vpn_conn_local:
        hyb_ops.del_vpc_vpn_gateway_local(conn.uuid)
    vpc_vpn_conn_local_after = hyb_ops.query_hybrid_eip_local()
    assert len(vpc_vpn_conn_local_after) == 0
