...
Create Resource Stack when state of stack template is false
(expected result is failed).
...
'''
    ...
    stack_template_queried = res_ops.query_resource(
            res_ops.STACK_TEMPLATE, cond)
    if len(stack_template_queried) == 0:
        test_util.test_fail("Fail to query stack template")

    stack_template_option.set_state('false')
    new_stack_template = stack_template_ops.update_stack_template(
            stack_template.uuid, stack_template_option)
    ...
    try:
        ...
        test_util.test_fail(
                'Cannot create stack when stack template is disabled.')
    except Exception:
        pass

    # 3.Qeury this stack
    cond = res_ops.gen_query_conditions('name', '=', 'Stack_test')
    resource_stack_queried = res_ops.query_resource(
    ...
    lastest_stack_template = stack_template_ops.update_stack_template(
            stack_template.uuid, stack_template_option)
    ...
    if resource_stack_queried[0].status == 'Created':
        if len(vm_queried) == 0:
            test_util.test_fail(
                    "Fail to create all resource when "
                    "resource stack status is Created")
