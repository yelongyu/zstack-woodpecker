...
class Maintain_Host_Parall(Host_Operation_Parall):
    def operate_host_parall(self, host_uuid):
        new_state = 'maintain'
        try:
            host_ops.change_host_state(host_uuid,new_state,self.session_uuid)
        except:
    ...
    def check_operation_result(self):
        for i in range(0, self.i):
            ...
            if v1.state != "Maintenance":
                test_util.test_fail('Fail to Maintain Host %s.' % v1.uuid)

def test():
    get_host_con = res_ops.gen_query_conditions('state', '!=', "Maintenance")
    maintain_hosts = Maintain_Host_Parall(get_host_con, "Maintenance")
    maintain_hosts.parall_test_run()
    maintain_hosts.check_operation_result()
