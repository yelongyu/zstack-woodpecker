...
class Enable_Host_Parall(Host_Operation_Parall):
    ...
    def check_operation_result(self):
        for i in range(0, self.i):
            ...
            if v1.state != "Enabled":
                test_util.test_fail('Fail to Enable Host %s.' % v1.uuid)

def test():
    get_host_con = res_ops.gen_query_conditions('state', '!=', "Enabled")
    enable_hosts = Enable_Host_Parall(get_host_con, "Enabled")
    enable_hosts.parall_test_run()
    enable_hosts.check_operation_result()
