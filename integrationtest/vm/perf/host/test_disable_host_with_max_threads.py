...
class Disable_Host_Parall(Host_Operation_Parall):
    ...
    def check_operation_result(self):
        for i in range(0, self.i):
            ...
            if v1.state != "Disabled":
                test_util.test_fail('Fail to Disable Host %s.' % v1.uuid)

def test():
    get_host_con = res_ops.gen_query_conditions('state', '!=', "Disabled")
    disable_hosts = Disable_Host_Parall(get_host_con, "Disabled")
    disable_hosts.parall_test_run()
    disable_hosts.check_operation_result()
