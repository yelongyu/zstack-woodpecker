...
class Reconnect_Host_Parall(Host_Operation_Parall):
    def operate_host_parall(self, host_uuid):
        try:
            host_ops.reconnect_host(host_uuid,self.session_uuid)
        except:
    ...
    def check_operation_result(self):
        for i in range(0, self.i):
            ...
            if v1.status != "Connected":
                test_util.test_fail('Fail to Reconnect Host %s.' % v1.uuid)

def test():
    get_host_con = res_ops.gen_query_conditions('status', '!=', "Connected")
    reconnect_hosts = Reconnect_Host_Parall(get_host_con, "Connected")
    reconnect_hosts.parall_test_run()
    reconnect_hosts.check_operation_result()
