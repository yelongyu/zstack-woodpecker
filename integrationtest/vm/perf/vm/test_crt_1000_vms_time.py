...
New Perf Test for creating 1000 vms and get the test used time..
...
def get_begin_time():
    ...
    for line in file_obj.readlines():
        if line.find('SimpleFlowChain') != -1 and line.find('create-vm') != -1 and line.find('starts') != -1:
...
def get_end_time():
    ...
    file_obj = open(mn_server_log)
    for line in file_obj.readlines():
        if line.find('event publish') != -1 and line.rfind('APICreateVmInstanceEvent') != -1:
