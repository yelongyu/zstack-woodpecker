...
def add_ps_network_gateway_sys_tag():
    ...
    if ps.type == "NFS":
        test_util.test_logger(
            "add system tag: resourceUuid=%s tag=%s" % (ps.uuid, "primaryStorage::gateway::cidr::10.0.0.1/8"))
...
def test():
    if test_lib.scenario_config != None and test_lib.scenario_file != None and not os.path.exists(
        ...
        scenario_operations.deploy_scenario(test_lib.all_scenario_config, test_lib.scenario_file,
    ...
    if test_lib.scenario_config != None and test_lib.scenario_file != None and os.path.exists(test_lib.scenario_file):
        mn_ips = deploy_operations.get_nodes_from_scenario_file(test_lib.all_scenario_config, test_lib.scenario_file,
                                                                test_lib.deploy_config)
