...
import zstackwoodpecker.operations.san_operations as san_ops
...
def test():
    ...
    lun = san_ops.get_scsi_lun_candidate_to_attach_vm(vm.get_vm().uuid)
 
    san_ops.attach_scsi_lun_to_vm(lun[0].uuid, vm.get_vm().uuid)
    ...
    cond = res_ops.gen_query_conditions("uuid", '=', lun[0].uuid) 
    assert vm.get_vm().uuid == res_ops.query_resource(res_ops.SCSI_LUN, cond)[0].scsiLunVmInstanceRefs[0].vmInstanceUuid
    print_path(Path)
