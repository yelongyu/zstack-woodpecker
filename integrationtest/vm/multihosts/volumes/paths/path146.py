import zstackwoodpecker.test_state as ts_header
TestAction = ts_header.TestAction
def path():
	return dict(initial_formation="template2", 
		path_list=[[TestAction.create_volume, "volume1", "=scsi"],\
		[TestAction.create_volume, "volume2", "=scsi"],\
		[TestAction.attach_volume, "vm1", "volume1"], \
		[TestAction.detach_volume, "volume1"], \
		[TestAction.create_image_from_volume, "vm1", "image1"], \
		[TestAction.create_volume_snapshot, "volume1", "snapshot51"], \
                [TestAction.create_volume_snapshot, "volume1", "snapshot52"], \
                [TestAction.create_volume_snapshot, "volume1", "snapshot53"], \
		[TestAction.attach_volume, "vm1", "volume1"], \
		[TestAction.detach_volume, "volume1"], \
		[TestAction.ps_migrate_volume, "volume1"],
		[TestAction.stop_vm, "vm1"], \
		[TestAction.ps_migrate_volume, "vm1-root"],
		[TestAction.start_vm, "vm1"], \
		[TestAction.attach_volume, "vm1", "volume1"], \
		[TestAction.stop_vm, "vm1"], \
		[TestAction.use_volume_snapshot, "snapshot51"], \
		[TestAction.start_vm, "vm1"], \
                [TestAction.batch_delete_volume_snapshot, ["snapshot51"]],
		[TestAction.detach_volume, "volume1"], \
		[TestAction.attach_volume, "vm1", "volume1"], \
		[TestAction.stop_vm, "vm1"], \
		[TestAction.reinit_vm, "vm1"], \
		[TestAction.use_volume_snapshot, "snapshot52"], \
		[TestAction.batch_delete_volume_snapshot, ["snapshot53"]],
		[TestAction.start_vm, "vm1"], \
		[TestAction.reboot_vm, "vm1"]])					
			

	
