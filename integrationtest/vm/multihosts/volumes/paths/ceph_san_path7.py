    ...
    if san_ps and ceph_ps:
        return dict(initial_formation="template3",
                    path_list=[[TestAction.create_volume, "ceph_shared_volume1", "=ps_uuid::%s,scsi,shareable" % ceph_ps[0]],
                               ...
                               [TestAction.create_volume_snapshot, san_vms[0].name + '-root', san_vms[-1].name + '-sp1'],
                               [TestAction.create_volume_snapshot, san_vms[1].name + '-root', san_vms[-1].name + '-sp1'],
                               [TestAction.create_volume_snapshot, san_vms[2].name + '-root', san_vms[-1].name + '-sp1'],
                               [TestAction.resize_volume, san_vms[-1].name, 5*1024*1024],
                               [TestAction.create_volume_snapshot, ceph_vms[-1].name + '-root', san_vms[-1].name + '-sp1'],
                               san_vms[-1].stop, 
                               ...
                               san_vms[2].migrate,
                               ceph_vms[-1].migrate,
                               [TestAction.create_volume_snapshot, san_vms[0].name + '-root', san_vms[-1].name + '-sp2'],
                               [TestAction.create_volume_snapshot, san_vms[1].name + '-root', san_vms[-1].name + '-sp2'],
                               [TestAction.create_volume_snapshot, san_vms[2].name + '-root', san_vms[-1].name + '-sp2'],
                               [TestAction.create_volume_snapshot, ceph_vms[-1].name + '-root', san_vms[-1].name + '-sp2'],
