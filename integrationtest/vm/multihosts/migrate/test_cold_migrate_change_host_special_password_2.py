...
New Integration test for testing stopped vm migration after change the password "Aa1~!@#$%^&*()?_+-{};:,.<>" between hosts.
...
#query all hosts and change password
    ...
    if res_ops.query_resource(res_ops.HOST, conditions):
        ...
        for i in kvm_host:
        	host_ops.update_kvm_host(i.uuid, 'password', 'Aa1~!@#$%^&*()?_+-{};:,.<>')
 		cmd = 'echo "Aa1~!@#$%^&*()?_+-{};:,.<>"| passwd --stdin root'
