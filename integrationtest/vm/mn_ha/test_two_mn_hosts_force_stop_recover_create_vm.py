...
Integration Test for creating KVM VM in MN HA mode with two random mn hosts force stop and recovery.
...
def test():
    ...
    for index in test_mn_host_list:
        test_util.test_logger("force stop host [%s]" % (mn_host_list[index].ip_))
        test_stub.stop_host(mn_host_list[index], test_lib.all_scenario_config, 'cold')
    ...
    test_util.test_logger("wait for 60 seconds to see if management node VM starts on any host")
    time.sleep(60)
    try:
        new_mn_host = test_stub.get_host_by_mn_vm(test_lib.all_scenario_config, test_lib.scenario_file)
        if len(new_mn_host) == 0:
            test_util.test_fail("management node VM does not start after recover one host and ha status is ready")
        elif len(new_mn_host) > 1:
            test_util.test_fail("management node VM starts on more than one host after recover one host and ha status is ready")
    except:
        test_util.test_fail("management node VM does not start after recover one host and ha status is ready")
