...
def test():
   ...
   #Hack inspect ks file to support vbmc, include ipmi device logic and ipmi addr to 127.0.0.1
    ...
    test_util.test_dsc('Add ubuntu image')
    ...
    img_option.set_url("http://192.168.200.100/mirror/iso/ubuntu-16.04-server-amd64.iso")
#    img_option.set_url(os.environ.get('imageServer')+"/iso/ubuntu-18.04.1-server-amd64.iso")
...
d-i netcfg/get_domain string localhost-domain
...
d-i pkgsel/include string openssh-server build-essential vim vlan

### NECESSARY: PRE SCRIPTS FOR ZSTACK
d-i preseed/early_command string {{ PRE_SCRIPTS }}

### NECESSARY: POST SCRIPTS FOR ZSTACK
d-i preseed/late_command string {{ POST_SCRIPTS }}
...
'''
    ...
    template = baremetal_operations.add_preconfiguration_template('ubuntu18', 'preseed', 'ubuntu-x86_64', content)


    test_util.test_dsc('Create baremetal instance')
    cond = res_ops.gen_query_conditions('name', '=', 'ubuntu18')
    ...
    customConfigurations={"timezone":"Asia/Hong_Kong"}
    baremetal_ins = test_stub.create_baremetal_ins(image_uuid, chassis_uuid, template.uuid, 'zstack', 'password', nic_cfgs, bonding_cfgs, customConfigurations, systemTags=tag)
...
#    vm_ip = vm_inv.vmNics[0].ip
    ...
    result = test_lib.lib_execute_ssh_cmd(ip_address1, 'zstack', 'password', cmd, 180)
    if 'Asia/Hong_Kong' not in result:
