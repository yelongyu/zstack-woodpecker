...
def test():
	...
	for i in range(0,vip_max):	
		...
		bill_vip_in = test_stub.PublicIpVipInBilling()
		time_unit = random.choice(time_unit_dict)
		price = str(random.randint(0,9999))
		resource_unit = random.choice(resource_unit_dict.keys())
		bill_vip_in.set_timeUnit(time_unit)
		bill_vip_in.set_price(price)
		bill_vip_in.set_resourceUnit(resource_unit)
		test_util.test_logger("create vip in  billing\n price=%s, timeUnit=%s" % (price, time_unit))
		bill_vip_in.create_resource_type()
		
		test_util.test_logger("create vip instance and set qos")
		global vip
		vip = test_stub.create_vip()
		qos_in = 1048576 #1M
		net_ops.set_vip_qos(vip.vip.uuid, inboundBandwidth = qos_in)	
		...
		for r in range(0,round_max):
			...
			if test_stub.check(bill_vip_in, billing_resource, random.choice(offset_unit_dict), random.randint(0,3), qos_size):
				success_round += 1
			else:
