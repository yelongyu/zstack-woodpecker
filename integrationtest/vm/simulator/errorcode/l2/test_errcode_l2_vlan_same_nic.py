'''
  attach 2 L2network with same nic name and vni to a cluster
...
def test():
    ...
    vlan_id = res_ops.query_resource(res_ops.L2_VLAN_NETWORK)[0].vlan
    nic_name = res_ops.query_resource(res_ops.L2_VLAN_NETWORK)[0].physicalInterface
    zone_uuid = res_ops.query_resource(res_ops.ZONE)[0].uuid
    cluster_uuid = res_ops.query_resource(res_ops.CLUSTER)[0].uuid
    l2_name = 'L2_vlan_test' 
    try:
        l2_uuid = net_ops.create_l2_vlan(l2_name, nic_name, zone_uuid, vlan_id).inventory.uuid
