'''
 CheckElaborationContent, elaborateFile=$empty_folder_path
 regex: "%s is not existed or is empty folder"
'''
import os
...
def test():
    ...
    for message in check_message_list:
        if regex == message.regex:
            check_message =  message.message_cn.encode('utf8').replace('%1$s', folder_path)
            break
    test_util.test_logger('@@@@DEBUG@@@@: %s' % check_message)
    #check elaborationcontent with empty_folder_path
    try:
         errc_ops.check_elaboration_content(folder_path)
