...
def test():
    ...
    vm3 = test_stub.create_ag_vm(affinitygroup_uuid=ag1.uuid, host_uuid=h3[0].uuid)
    assert vm3.get_vm().hostUuid == h3[0].uuid
    test_obj_dict.add_vm(vm3)

    try:
        vm1.migrate(vm3.get_vm().hostUuid)
    except:
        test_util.test_logger("vm3 is not expected to migrate to host3 [uuid: %s]" % vm3.get_vm().hostUuid)
    
    ag_ops.remove_vm_from_affinity_group(affinityGroupUuid=ag1.uuid, vm_uuid=vm3.get_vm().uuid)
    vm1.migrate(vm3.get_vm().hostUuid)
