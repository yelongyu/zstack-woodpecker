...
def test():
    ...
    new_vm = vm3.clone(names=["clone-vm"], systemtag=["affinityGroupUuid::%s" % ag1.uuid])
    ...
    ag_ops.remove_vm_from_affinity_group(affinityGroupUuid=ag1.uuid, vm_uuid=new_vm[0].get_vm().uuid)
    ag_ops.add_vm_to_affinity_group(ag1.uuid, vm1.get_vm().uuid)
    vmuuids = []
    ag = test_lib.lib_get_affinity_group_by_name(name="ag1")
    for usage in ag.usages:
