...
def test():
    ...
    test_stub.subscribe_sns_topic(my_sns_topic_uuid, email_endpoint_uuid)

    host_sns_topic = test_stub.create_sns_topic('host_topic')
    host_sns_topic_uuid = host_sns_topic.uuid
    test_stub.subscribe_sns_topic(host_sns_topic_uuid, email_endpoint_uuid)
    ...
    ps_event_sub_uuid = test_stub.subscribe_event(ps_namespace, ps_disconnected, ps_actions).uuid
    ...
    bs_event_sub_uuid = test_stub.subscribe_event(bs_namespace, bs_disconnected, bs_actions).uuid
    ...
    host_status_event_sub_uuid = test_stub.subscribe_event(host_namespace, host_status_changed, host_actions, host_status_labels).uuid

    host_disconnected = 'HostDisconnected'
    host_disconnected_event_sub_uuid = test_stub.subscribe_event(host_namespace, host_disconnected, host_actions).uuid

    if test_stub.check_sns_email(pop_server, username, password, ps_disconnected, ps_event_sub_uuid):
        test_util.test_fail('email already exsist before test')
    if test_stub.check_sns_email(pop_server, username, password, bs_disconnected, bs_event_sub_uuid):
        test_util.test_fail('email already exsist before test')
    if test_stub.check_sns_email(pop_server, username, password, host_status_changed, host_status_event_sub_uuid):
        test_util.test_fail('email already exsist before test')
    if test_stub.check_sns_email(pop_server, username, password, host_disconnected, host_disconnected_event_sub_uuid):
        test_util.test_fail('email already exsist before test')
    ...
    host_ops.reconnect_host(host_uuid)
    test_stub.delete_sns_topic(host_sns_topic_uuid)
    test_stub.delete_sns_topic(my_sns_topic_uuid)
    ...
    check_1 = test_stub.check_sns_email(pop_server, username, password, ps_disconnected, ps_event_sub_uuid)
    check_2 = test_stub.check_sns_email(pop_server, username, password, bs_disconnected, bs_event_sub_uuid)
    check_3 = test_stub.check_sns_email(pop_server, username, password, host_status_changed, host_status_event_sub_uuid)
    check_4 = test_stub.check_sns_email(pop_server, username, password, host_disconnected, host_disconnected_event_sub_uuid)
