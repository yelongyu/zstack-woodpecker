...
def test():
	...
	all_metric_metadata=test_stub.get_all_metric_metadata()
	test_util.test_dsc('total account of metric metadata:%s'%len(all_metric_metadata))
	all_event_metadata=test_stub.get_all_event_metadata()
	...
	for metric_metadata in all_metric_metadata:
		if metric_metadata.labelNames:
			...
			# test_util.test_dsc('\n[namespace]:%s [name]:%s [labelNames]:%s '%(metric_metadata.namespace,metric_metadata.name,metric_metadata.labelNames))
		else:
	...
	for metric_metadata in metric_metadata_with_label_list:
		if len(metric_metadata.labelNames)== 1:
			labels = test_stub.get_metric_label_value(metric_metadata.namespace, metric_metadata.name, metric_metadata.labelNames)
		...
		if len(metric_metadata.labelNames)== 2:
			...
			# for label in labels:
			# 	test_util.test_logger(' [%s] : %s\n [%s]: %s\n'%(metric_metadata.labelNames[0],label[metric_metadata.labelNames[0]],metric_metadata.labelNames[1],label[metric_metadata.labelNames[1]]))

	#test GetMetricData
	for metric_metadata in all_metric_metadata:
		data_list=test_stub.get_metric_data(metric_metadata.namespace,metric_metadata.name)
		# for data in data_list:
		# 	test_util.test_logger('%s:[labels]:%s [time]:%s [value]:%s'%(metric_metadata.name,data.labels,data.time,data.value))
	...
	test_stub.put_metric_data(my_namespace,my_data)

	#wait for data
	time.sleep(60)
	my_data_get=test_stub.get_metric_data(my_namespace,my_metric_name)
	if not my_data_get:
	...
	# 	for event in events:
	# 		test_util.test_logger('[time]:%s,[namespace]:%s ,[name]:%s,[labels]:%s,[resourceId]:%s'%(event.time,event.namespace,event.name,event.labels,event.resourceId))

	#test GetAuditData
	current_time=int(round(time.time()*1000))
	audits=test_stub.get_audit_data(start_time=begin_time,end_time=current_time)
	flag=0
