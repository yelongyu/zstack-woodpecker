...
def full_name(lns, fns):
    ...
    _first = first_name(random.randint(1, 2), _last, fns)

    en_name = pinyin.get("%s.%s"%(_first, _last), format="strip")
    ch_name = _last+_first
    return (en_name, ch_name)
...
def generate_user_info():
    ...
    (_name_en, _name_ch) = full_name(last_names, first_names)
    _email = _name_en+"@zstack.io"
    _phone_num = random.choice(['139','135', '138', '188','185','136','158','151'])+"".join(random.choice("0123456789") for i in range(8))
    return (_name_ch, _email, _phone_num)

if __name__ == "__main__":
    (name, email, phone) = generate_user_info()
    print name
    print email
    print phone
