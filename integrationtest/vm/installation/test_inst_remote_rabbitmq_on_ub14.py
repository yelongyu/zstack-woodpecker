...
def test():
    ...
    test_util.test_dsc('Create 2 Ubuntu14.04 vm to test zstack installation. Rabbitmq server is on different node with zstack management node')
    ...
    cmd = '%s "zstack-ctl install_management_node --host=%s --install-path=/usr/local/zstack"' % (ssh_cmd1, vm2_ip)
    process_result = test_stub.execute_shell_in_process(cmd, tmp_file)
    if process_result != 0:
    ...
    if process_result != 0:
        if 'no management-node-ready message received within' in open(tmp_file).read():
            ...
            while (times > 0):
                ...
                times -= 1
                if process_result == 0:
                    test_util.test_logger("management node1 start after extra %d seconds" % (30 - times + 1) * 10 )
                    break
            else:
