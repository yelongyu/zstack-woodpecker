...
def test():
    ...
    if test_stub.test_fio_bandwidth(vm_inv, read_bandwidth, raise_exception=False):
        test_util.test_fail('disk write qos is not expected to have limit as only read qos was set')

    if test_stub.test_fio_bandwidth(vm_inv, read_bandwidth, path, raise_exception=False):
        test_util.test_fail('disk write qos is not expected to have limit as only read qos was set')
