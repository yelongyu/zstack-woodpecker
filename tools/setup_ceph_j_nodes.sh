...
if [ "${CEPH_ONE_NODE}" != "yes" -a "$CEPH_TWO_NODES" != "yes" ]; then
    echo " ${IP[1]} ceph-2 ">>/etc/hosts
    echo " ${IP[2]} ceph-3 ">>/etc/hosts
elif [ "$CEPH_TWO_NODES" == "yes" ];then
    echo " ${IP[1]} ceph-2 ">>/etc/hosts
fi
...
if [ "${CEPH_ONE_NODE}" != "yes"  -a "$CEPH_TWO_NODES" != "yes" ]; then
	...
	ssh ceph-3 'chown -R ceph:ceph /dev/vdb'
elif [ "$CEPH_TWO_NODES" == "yes" ];then
	ssh ceph-2 'chown -R ceph:ceph /dev/vdb'
fi
...
for I in `seq 3`; do
	if [ "${CEPH_ONE_NODE}" != "yes"  -a "$CEPH_TWO_NODES" != "yes" ]; then
		...
		ceph-deploy --overwrite-conf mon create ceph-1 ceph-2 
	else
	...
	if [ "${CEPH_ONE_NODE}" != "yes"  -a "$CEPH_TWO_NODES" != "yes" ]; then
		...
		ceph-deploy gatherkeys ceph-1 ceph-2 
	else
	...
	if [ "${CEPH_ONE_NODE}" != "yes"  -a  "$CEPH_TWO_NODES" != "yes" ]; then
		...
		ceph-deploy --overwrite-conf osd create ceph-1:/dev/vdb ceph-2:/dev/vdb 
	else
